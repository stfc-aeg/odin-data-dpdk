...
class LiveDataProcessor():
    ...
    def __init__(self, endpoint, resolution, size_x=2748, size_y=1836, colour='bone'):
        ...
        # self.frame_num = 0
        # frame number in live_data adapter or camera controls? - react expecting in live_data
        self.data_min = 0
        self.data_max = 255
    ...
    def read_data_from_socket(self, msg):
        ...
        if len(msg[1]) != (self.max_size_x*self.max_size_y*2):
            ...
            # data = np.frombuffer(uncompressed_data, dtype=dtype)
            data = np.frombuffer(msg[1], dtype=dtype)
        else:
            data = np.frombuffer(msg[1], dtype=dtype)  # Otherwise, grab the data as-is
            
        # print("Raw data stats:", data.shape, data.dtype, np.min(data), np.max(data))
        ...
        reshaped_data = clipped_data.reshape((3672, 5496))
        ...
        # print("ROI:", self.roi)
        # print("ROI slice:", self.roi['y_lower'], self.roi['y_upper'], self.roi['x_lower'], self.roi['x_upper'])


        # roi_data = resized_data[self.roi['y_lower']:self.roi['y_upper'],
        #                 self.roi['x_lower']:self.roi['x_upper']]
        
        x_scale = self.size_x / self.max_size_x
        y_scale = self.size_y / self.max_size_y

        x_lower = int(self.roi['x_lower'] * x_scale)
        x_upper = int(self.roi['x_upper'] * x_scale)
        y_lower = int(self.roi['y_lower'] * y_scale)
        y_upper = int(self.roi['y_upper'] * y_scale)

        roi_data = resized_data[y_lower:y_upper, x_lower:x_upper]

        colour_data = cv2.applyColorMap((roi_data).astype(np.uint8), self.get_colour_map())
        ...
        self.image_queue.put(png_bytes)
