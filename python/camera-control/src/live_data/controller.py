...
from live_data.util import LiveXError
from live_data.processor import LiveDataProcessor
from live_data.base_controller import BaseController
# from live_data.base_controller import BaseError
...
class LiveDataController(BaseController):
    ...
    def __init__(self, options): # endpoints, names, resolutions):
        ...
        for colormap_name in sorted(self.cv2_colourmaps.keys()):
        ...
        for i in range(len(endpoints)):
            ...
            proc = LiveDataProcessor(endpoints[i], resolution)
            proc.name = name
            ...
            tree = {
                ...
                # "image_data": (lambda proc=proc: proc.get_image(), None),
                "image_data": (lambda: "Image data not available in JSON", None),
                "image_metadata":
                {  # Partials provide processor as an argument
                    ...
                    "colour_options": self.colormap_options,
                    ...
                    # "frame_num": (lambda proc=proc: proc.frame_num, None),
                    "data_min_max": (lambda proc=proc: [proc.data_min, proc.data_max], None)
                }
            }
    ...
    def _update_render_info(self, processor):
        ...
        params = {
            ...
            "data_min": processor.data_min, #
            "data_max": processor.data_max, #
            "roi": processor.roi
        }
    ...
    def set_img_colour(self, value, processor):
        ...
        :param processor: LiveDataProcessor object to reference
        """
        logging.debug("set_img_colour")
        logging.debug("Colour: " + value)
